...
public class LuggageDAO implements ILuggage {
    ...
    public static final String GET_ALL_BY_ID  = "SELECT * FROM Luggage WHERE Passengers_id = ?";
            ...
            try {
                ...
                try {
                    ...
                    if (rSet.next()) {
                        ...
                        luggage.setId(rSet.getLong("Passengers_id"));
                        return luggage;
                    }
                } finally {
                    try {
    ...
    public Luggage save(Luggage luggage) {
        ...
        try {
            ...
            try {
                ...
                prStatement.setLong(3, luggage.getId());
    ...
    @Override
    public List <Luggage> getAllId(Long id) {
        ...
        try {
            ...
            try {
                logDAO.trace("Create PreparedStatement");
                prStatement = connection.prepareStatement(GET_ALL_BY_ID);
                try {
                    logDAO.trace("Get ResultSet");
                    rSet = prStatement.executeQuery();
                    List<Luggage> luggag = new ArrayList<>();
                    while (rSet.next()) {
                        ...
                        luggage.setBarcode(rSet.getLong("barcode"));
                        luggage.setWeight(rSet.getLong("weight"));
                        luggage.getPassenger().setId(rSet.getLong("passenger_all_id"));
                        luggag.add(luggage);
